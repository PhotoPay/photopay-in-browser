...
function main() {
    ...
    // 3. Load SDK
    PhotoPaySDK.loadWasmModule(loadSettings).then((sdk: PhotoPaySDK.WasmSDK) => {
...
async function startScan(sdk: PhotoPaySDK.WasmSDK) {
    ...
    const videoRecognizer = await PhotoPaySDK.VideoRecognizer.createVideoRecognizerFromCameraStream(cameraFeed, recognizerRunner);
    ...
    if (processResult !== PhotoPaySDK.RecognizerResultState.Empty) {
        ...
        if (croPdf417Result.state !== PhotoPaySDK.RecognizerResultState.Empty) {
            ...
            alert(`Please pay ${croPdf417Result.amount}${croPdf417Result.currency} to recipient ${croPdf417Result.recipientName}, account number: ${croPdf417Result.iban} with payment reference ${croPdf417Result.referenceModel} ${croPdf417Result.reference}!`);
        }
    }
    else {
